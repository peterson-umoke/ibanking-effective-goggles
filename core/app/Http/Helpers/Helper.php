...
if (!function_exists('send_sms')) {
    ...
    function send_sms($to, $message)
    {
        ...
        if ($gnl->sms_notification == 1) {
            ...
            $appi = $gnl->sms_api;
            $appi = str_replace(array("{{number}}", "{{message}}"), array($to, $sendtext), $appi);
...
if (!function_exists('ads_show')) {
    ...
    {
        $ads = Advertisement::where('size', $size)->where('status', 1)->inRandomOrder()->first();
        if (!empty($ads)) {
...
if (!function_exists("generateSalt_e")) {
    function generateSalt_e($length)
    {
        ...
        for ($i = 0; $i < $length; $i++) {
            $random .= $data[(mt_rand() % (strlen($data)))];
        }
...
if (!function_exists("callAPI")) {
    function callAPI($apiURL, $requestParamList)
    {
        ...
        try {
            $responseParamList = json_decode($jsonResponse, true, 512, JSON_THROW_ON_ERROR);
